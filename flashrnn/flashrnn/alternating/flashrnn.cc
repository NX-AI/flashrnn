...
public:
  FlashRNNFunc(const bool training, const int batch_size, const int hidden_size,
               const int num_heads)
  ...
  std::vector<Tensor> forward(bool training, Tensor x, Tensor s0,
    ...
    gate_cache_r = torch::empty(
        {time_steps, batch_size, num_heads, FLASHRNN_NUM_GATES_R, head_dim},
    ...
    gate_cache_i = torch::empty(
        {time_steps, batch_size, num_heads, FLASHRNN_NUM_GATES_I, head_dim},
        options.dtype(typeToTorchDtype<FLASHRNN_DTYPE_G>()));
#endif
    Tensor tmp_Ry =
        torch::empty({batch_size, hidden_size * FLASHRNN_NUM_GATES_R},
                     options.dtype(typeToTorchDtype<FLASHRNN_DTYPE_G>()));
  ...
  std::vector<Tensor> backward(Tensor recurrent_kernel_t, Tensor bias, Tensor s,
    ...
    Tensor dR = torch::zeros(
        {FLASHRNN_NUM_HEADS, head_dim, FLASHRNN_NUM_GATES_R, head_dim},
        options);
    ...
    Tensor gate_cache_bias =
        torch::ones({}, options.dtype(typeToTorchDtype<FLASHRNN_DTYPE_G>()));
#else
    // this is for both recurrent and input base caches, without additional
    // bias-only gates
    Tensor gate_cache_bias = torch::ones(
