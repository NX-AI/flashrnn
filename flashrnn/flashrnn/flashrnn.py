...
class FlashRNNConfig:
    ...
    def __post_init__(self):
        ...
        if self.backend == "vanilla":
            ...
            if self.dtype == "float32":
                self.batch_size = 16
        elif self.backend == "cuda_fused_custom":
            self._internal_input_shape = "TBHDG"
            self._internal_recurrent_shape = "HDGP"
            self._internal_bias_shape = "HDG"
            self._internal_output_shape = "STBHD"
...
class _FlashRNNCUDAFusedCustom:
    mod: dict = {}
    ...
    def instance(cls, config: FlashRNNConfig):
        ...
        if cfgdevstr not in cls.mod:
            ...
            name = config.function + "_f"
            sources = [
                str(curdir / "fused_custom" / "flashrnn.cc"),
                str(curdir / "fused_custom" / "flashrnn_fused_forward.cu"),
                str(curdir / "fused_custom" / "flashrnn_fused_backward.cu"),
                str(curdir / "fused_custom" / "flashrnn_fused_backward_cut.cu"),
                str(curdir / "util" / "blas.cu"),
                str(curdir / "util" / "cuda_error.cu"),
            ]
            seq_len = 2
            module = load_parametric_and_test_and_bisect(
                ...
                constraint_str=constraint_str,
                value_refinements=value_refinements,
                model_class="FlashRNNFuncFusedCustom",
                model_args=(
                    True,
                    config.batch_size,
                    config.hidden_dim,
                ...
                extra_cuda_cflags=[
                    "-include",
                    str(curdir / "fused_custom" / f"{config.function}_fused_pointwise.cuh"),
                ],
            )
            cls.mod[cfgdevstr] = module.FlashRNNFuncFusedCustom(
                True,
                config.batch_size,
                config.hidden_dim,
                config.num_heads,
...
def FlashRNNFuncGeneratorFusedCustom(training, config: FlashRNNConfig):
    flashrnn_cuda = _FlashRNNCUDAFusedCustom.instance(config)
    # pad batch size to multiple of 8
    round_batch_size = (
        round_to_multiple(config.batch_size, 8 if config.dtype != "float32" else 16)
        if config.dtype != "float32"
    ...
    class FlashRNNFunctionFusedCustom(torch.autograd.Function):
        @staticmethod
        @conditional_decorator(
            config.enable_automatic_mixed_precision,
            torch.amp.custom_fwd(
    ...
    return FlashRNNFunctionFusedCustom
...
def _get_kernel(config: FlashRNNConfig):
    ...
    elif config.backend == "cuda_fused_custom":

        def fn(Wx, states, R, b, **kwargs):
            states = FlashRNNFuncGeneratorFusedCustom(
                torch.is_grad_enabled(), config=config
            ).apply(
                torch.is_grad_enabled(),
                Wx.contiguous(),
